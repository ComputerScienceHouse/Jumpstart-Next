...
class OpenWeatherWrapper:
    def __init__(self, api, latitude, longitude, units="imperial", update=300) -> None:
        self.api = api
        self.lat = latitude
        self.lon = longitude
        self.units = units
    ...
    def start(self):
        self.thread = threading.Thread(
            name="thread_jumpstart-next_component_info_weather",
...
InfoComponentRouter = APIRouter(
    prefix="/api/components/info", tags=["component", "info"]
)
...
async def get_weather(r: Response):
    ...
    if weather_data:
        ...
        return {"result": "success", "data": weather_data}
    else:
