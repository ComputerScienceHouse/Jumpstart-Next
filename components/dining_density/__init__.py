...
class OpenFoodWrapper:
    ...
    def start(self):
        threading.Thread(
            name="thread_jumpstart-next_component_dining-density_food",
    ...
    def fetchLoop(self):
        while True:
            try:
                data = requests.get(
                    "https://maps.rit.edu/proxySearch/densityMap.php"
                ).json()["busyness"]
                data = data[
                    :
                ]  # Food places are from 0-16, unfortunate hardcoding...update later
                for i in range(len(data)):
                    data[i] = data[i]["properties"]
                    data[i]["pct_full"] = int(
                        100 * data[i]["count"] / data[i]["max_occ"]
                    )
                    data[i].pop("mdo_id")
...
DiningDensityComponentRouter = APIRouter(
    prefix="/api/components/dining-density", tags=["component", "dining-density"]
)
...
async def get_food(r: Response):
    ...
    if food_data:
        del food_data["_id"]
        return {"result": "success", "data": food_data}
    else:
